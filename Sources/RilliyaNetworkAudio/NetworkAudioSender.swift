...
public struct NetworkAudioSenderConfiguration: Sendable {
  ...
  /// scheduler.
  var realtimeComputationBudget: Duration {
    guard let codec = NetworkAudioCodec.codec(for: encoding) else {
      return Self.datagramBudget
    }
  ...
  private func start(sharedKey: NetworkAudioSharedKey?) throws {
    ...
    let worker = AudioRealtimeWorker(
      ...
      budget: try AudioRealtimeBudget.matching(
        computation: configuration.realtimeComputationBudget
      )
    ) { _ in
